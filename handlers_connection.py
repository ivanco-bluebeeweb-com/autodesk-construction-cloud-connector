"""Connection lifecycle for Autodesk Construction Cloud Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from autodesk_construction_cloud_client import AutodeskConstructionCloudClient
...
_SECRET = "autodesk_construction_cloud_connections"
...
@chat.function(
    "connect_autodesk_construction_cloud",
    "Connect Autodesk Construction Cloud account via credentials.",
    action_type="write",
    chain_callable=True,
    event="autodesk-construction-cloud-connector.connect_autodesk_construction_cloud",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_autodesk_construction_cloud(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Autodesk Construction Cloud Connector."""
    client = AutodeskConstructionCloudClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Autodesk Construction Cloud Account",
...
@chat.function(
    "list_connections",
    "List connected Autodesk Construction Cloud accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_autodesk_construction_cloud",
    "Disconnect Autodesk Construction Cloud account.",
    action_type="write",
    chain_callable=True,
    event="autodesk-construction-cloud-connector.disconnect_autodesk_construction_cloud",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_autodesk_construction_cloud(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
